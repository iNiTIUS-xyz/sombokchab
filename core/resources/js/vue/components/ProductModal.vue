          ...
          <div class="row g-4">
            <div class="col-lg-12">
              <div class="dashboard_posSystem__category__nav__inner">
                <p class="dashboard_posSystem__category__nav__item">{{ product.product.category.name }}</p>
                <p class="dashboard_posSystem__category__nav__item">{{ product.product.sub_category.name }}</p>
                <p class="dashboard_posSystem__category__nav__item">{{ childCategoryString(product.product.child_category) }}</p>
              </div>
            </div>
            <div class="col-lg-4">
              <div class="editProduct__thumb">
                <div class="editProduct__thumb__main">
                  <img v-bind:src="productImage" alt="" id="product-image-field" v-bind:data-src="product.product.image">
                </div>
              </div>

              <div class="shop-details-thumb-wrapper text-center d-flex gap-2 align-content-center justify-content-center">
                <div class="shop-details-thums bg-item-five" v-for="productGallery in product.product.gallery_images">
            ...
            <div class="col-lg-8">
              <div class="editProduct__contents">
                <h5 class="editProduct__contents__title"><a href="{{ product.product_url }}"> {{ product.product.name }}</a></h5>
                <p>
                ...
                <div class="valueInput value-input-area mt-4" v-if="(product?.product?.size?.length ?? 0) > 0">
                  <span class="valueInput__head">
                      <strong> Size: </strong>
                      <input class="form--input value-color" name="color" type="text" value="">
                      <input type="hidden" id="selected_size">
                  </span>
                  <ul class="valueInput__list size-lists" data-type="Size">
                    <li @click="handleSizeListClick" class="text-center" v-for="size in product.product.size"
                ...
                <div class="valueInput value-input-area mt-4" v-if="(product?.productColors?.length ?? 0) > 0">
                  <span class="valueInput__head">
                      <strong> Color: </strong>
                      <input class="form--input value-color" name="color" type="text" value="">
                  ...
                  <ul class="valueInput__list size-lists" data-type="Color">
                    <li @click="handleSizeListClick" class="text-center" v-for="color in product.productColors"
                ...
                <div class="value-input-area margin-top-15 attribute_options_list" v-for="(attribute, attr_name) in product.available_attributes">
                ...
                <div class="btn-wrapper btn_flex justify-content-start mt-4">
                  <div class="product__quantity m-0 radius-5">
                      <span class="substract" @click="productQuantity > 1 ? productQuantity-- : productQuantity">
                        <i class="las la-minus"></i>
                      </span>
                    <input class="product__quantity__input radius-5" type="number" v-model="productQuantity">
                    <span class="plus" @click="productQuantity++">
                        <i class="las la-plus"></i>
                      </span>
                  </div>
                  <a href="#1" class="posBtn btn_bg_1 radius-5" @click="variantProductAddToCart(product.product)">Add to cart</a>
                  <div class="text-success mt-2" v-if="variantStock > 0"> In Stock ({{ variantStock }}) </div>
...
export default {
  ...
  setup: (props,{emit}) => {
    ...
    function variantProductAddToCart(product){
      let selected_size = document.querySelector('#selected_size')?.value ?? null;
      let selected_color = document.querySelector('#selected_color')?.value ?? null;
      ...
      if (validateSelectedAttributes()) {
        ...
        send_ajax_request("post", data ,window.appUrl + "/shop-page/cart/ajax/add-to-cart", ()=> {},(data) => {
          //todo:: emit an event
          emit('cartAdded');
