...
function handleCountryState(event) {
    ...
    if (country_id !== '') {
        axios.get(window.appUrl + "/api/v1/get-states/" + country_id).then((response) => {
...
function handleStateCity(event) {
    ...
    if (state_id !== '') {
        axios.get(window.appUrl + "/api/v1/get-cities/" + state_id).then((response) => {
...
function checkUsernameIsAvailableOrNot(event) {
    ...
    axios.post(window.appUrl + "/api/v1/username", {
        username: username
    }).then((response) => {
