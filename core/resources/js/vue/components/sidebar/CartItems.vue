...
export default {
    ...
    setup: (props, {emit}) => {
        ...
        function taxAmount() {
            axios.get(window.appUrl + "/shop-page/cart/ajax/tax-amount").then((response) => {
                tax.value = 0;
            });
        }
        ...
        function getTotalAmountWithTax(response) {
            ...
            Object.keys(response).forEach(function (key) {
                ...
                tax += response[key]?.options?.tax_options_sum_rate ?? 0;

                let product_tax = response[key].options?.tax_options_sum_rate ?? 0;
