        ...
        if ($('#dataTable').length) {
            new DataTable('#dataTable', {
                ...
                language: {
                    ...
                    search: "{{ __('Filter:') }}",
                    zeroRecords: "{{ __('No data available') }}",
            ...
            if (!input_values.length) {
                ...
                $.get('{{ route('frontend.products.search') }}', {
                    ...
                    if (fetchedProducts.length) {
                        ...
                        let html = '';
                        const IN_STOCK = "{{ __('In Stock') }}";
                        const OUT_OF_STOCK = "{{ __('Out of Stock') }}";

                        fetchedProducts.forEach(function(product) {
                            const stockText = product.stock_count > 0 ? IN_STOCK : OUT_OF_STOCK;
