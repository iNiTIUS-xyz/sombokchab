<form action="{{ route('frontend.ajax.products.search') }}" method="get" id="search_product" style="display: none">
    ...
    <input type="hidden" id="category" name="category"
        value="{{ request()->id ? Modules\Attributes\Entities\Category::find(request()->id)->name ?? '' : '' }}">
    <input type="hidden" id="sub_category" name="sub_category"
        value="{{ request()->sub_cat_id ? Modules\Attributes\Entities\SubCategory::find(request()->sub_cat_id)->name ?? '' : '' }}">
