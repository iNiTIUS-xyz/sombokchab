...
@section('style')
    ...
    <style>
        ...
        .shippingMethod__wrapper__item span.zone { margin: -5px !important; }
...
@section('content')
        ...
        <div class="checkout-area-wrapper padding-top-20 padding-bottom-20">
            <div class="container">
                ...
                <form action="{{ route('frontend.checkout') }}"
                      class="billing-form checkout-billing-form"
                      method="POST"
                      id="billing_info"
                        ...
                        <div class="col-md-7 col-lg-7">
                            ...
                            <div class="checkout-inner-content">
                                <div class="billing-details-area-wrapper">
                                    <h4 class="title mb-3">{{ __('Shipping details') }}</h4>

                                    @if (auth('web')->check())
                                        <div class="btn-wrapper">
                                            <button type="button"
                                                    class="cmn-btn btn-bg-1 billing_details_click btn-small">
                                    ...
                                    <style>
                                        ...
                                        .option-card {
                                            ...
                                            padding: .65rem 1rem; border: 1px solid #d9d9d9;
                                            border-radius: .5rem; background: #fff; cursor: pointer;
                                            transition: border-color .2s ease, background-color .2s ease; width: 100%;
                                        }
                                        .option-input { position: absolute; opacity: 0; pointer-events: none; }
                                        .option-card:has(.option-input:checked) { border: 1px solid var(--main-color-one); }
                                        .option-radio {
                                            width: 22px; height: 22px; border: 2px solid #bfbfbf; border-radius: 50%;
                                            display: inline-block; position: relative; flex: 0 0 22px;
                                        }
                                        .option-radio::after {
                                            content: ""; position: absolute; inset: 3px; border-radius: 50%;
                                            background: var(--main-color-two); transform: scale(0); transition: transform .15s ease;
                                        }
                                        ...
                                        .option-badge {
                                            ...
                                            border-radius: .35rem; background: #111; color: #fff;
                                        }
                                        .option-card:hover { border-color: var(--main-color-one); box-shadow: 0 0 0 3px rgba(30, 123, 133, .08); }
                                        .option-input:checked + .option-radio { border-color: var(--main-color-one); }
                                        .option-input:checked + .option-radio::after { transform: scale(1); }
                                        .option-input:checked ~ .option-text .option-title { color: var(--main-color-one); }
                                        .option-input:checked ~ .option-text .option-sub { color: #444; }
    ...
    <div class="checkout-inner-content popup_modal_checkout">
        <div class="billing-details-area-wrapper position-relative">
            ...
            <form action="{{ route('frontend.checkout') }}" class="billing-form" method="POST" id="shopping_address" enctype="multipart/form-data">
                @csrf
    ...
    <script>
        function initialize() {
            ...
            google.maps.event.addListener(autocomplete, "place_changed", function() {
                ...
                document.getElementById("address").value = place.formatted_address ? place.formatted_address : (place.name || "");
            });
    ...
    <script>
        $(document).on("submit", "#shopping_address", function(e) {
            ...
            send_ajax_request(
                ...
                function(xhr) { ajax_toastr_error_message(xhr); }
            )
        });
