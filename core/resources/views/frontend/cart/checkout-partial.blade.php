...
<div class="cost-name-amount sub-total-wrap">
    <span class="total">
        {!! filter_static_option_value('subtotal_text', $setting_text, __('sub total')) !!}:
    </span>
    <span class="total-amount" id="subtotal" data-amount="{{ $subtotal }}">
...
<div class="cost-name-amount sub-total-wrap shipping-container">
    <div class="shipping-option-container">
        @if (isset($default_shipping) && $default_shipping->id)
            <div class="cost-name-amount all-shipping-options">
                ...
                <span class="same sub-amount">
                    {{ float_amount_with_currency_symbol(optional($default_shipping->availableOptions)->cost ?? 0) }}
                </span>
            </div>
        @endif
    </div>
    ...
    <div class="shipping-cost">
        <span class="total shipping">
            {!! filter_static_option_value('shipping_text', $setting_text, __('shipping')) !!}:
        </span>
        <span class="total-amount" id="shipping_charge">
...
<div id="discount_summery" style="display: none">
    ...
    <div class="cost-name-amount sub-total-wrap">
        <span class="total discount">
            {!! filter_static_option_value('discount_text', $setting_text, __('discount')) !!}
            <span class="ex">
                {!! filter_static_option_value('coupon_text', $setting_text, __('coupon')) !!}
            </span>
        </span>
        <strong>
...
<div class="cost-name-amount total-wrap">
    <span class="total">
        {!! filter_static_option_value('total_text', $setting_text, __('Total')) !!}:
    </span>
    <span class="total-amount">
        <span class="ex" id="total_amount">
