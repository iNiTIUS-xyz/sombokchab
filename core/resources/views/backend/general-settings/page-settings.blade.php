    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="dashboard__card">
                ...
                <div class="dashboard__card__body custom__form mt-4">
                    ...
                    <form action="{{ route('admin.general.page.settings') }}" method="POST"
                        ...
                        <div class="row">
                            <div class="col-lg-6">
                                @php
                                $all_page_slug_settings = ['about_page', 'product_page', 'faq_page', 'blog_page',
                                'contact_page'];
                                @endphp
                                <div class="row">
                                    ...
                                    <div class="col-lg-6">
                                        <div class="from-group margin-bottom-30">
                                            <label for="{{ $slug_field }}_slug">{{ ucfirst(str_replace('_', ' ',
                                                $slug_field)) }}
                                                {{ __('slug') }}</label>
                                            <input type="text" class="form-control"
                                                value="{{ get_static_option($slug_field . '_slug') }}"
                                                name="{{ $slug_field . '_slug' }}" placeholder="{{ __('Slug') }}">
                                            ...
                                            <div class="dashboard__card">
                                                <div class="dashboard__card__header">
                                                    <button class="btn btn-link collapsed" type="button"
                                                        ...
                                                        aria-expanded="false">
                                                        <span class="page-title">{{ get_static_option($slug_field .
                                                            '_name') ?? ucfirst(str_replace(['_', '-page'], [' ', ''],
                                                            $slug_field)) }}</span>
                                                    </button>
                                                </div>
                                                <div id="{{ $slug_field . '_content' }}" class="collapse"
                                                    ...
                                                    <div class="dashboard__card__body">
                                                        ...
                                                        <div class="form-group margin-top-20">
                                                            <label for="{{ $slug_field }}_meta_tags">{{ __('Meta Tags')
                                                                }}</label>
                                                            <input type="text" name="{{ $slug_field }}_meta_tags"
                                                                class="form-control" data-role="tagsinput"
                                                                value="{{ get_static_option($slug_field . '_meta_tags') }}">
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="{{ $slug_field }}_meta_description">{{ __('Meta
                                                                Description') }}</label>
                                                            <textarea name="{{ $slug_field }}_meta_description"
                                                                class="form-control"
                                                                rows="5">{{ get_static_option($slug_field . '_meta_description') }}</textarea>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                ...
                <button id="update" type="submit" class="cmn_btn btn_bg_profile">{{ __('Update') }}</button>
                </form>
            </div>
        </div>
    </div>
