...
<div class="form-group">
    <label for="add_remove_comman_form_amount">{{__('Amount format by')}}</label>
    <select name="amount_format_by" class="form__control radius-5">
        <option value="," {{get_static_option('amount_format_by') == ',' ? 'selected' : ''}}>{{ __('Comma') }}</option>
        <option value="." {{get_static_option('amount_format_by') == '.' ? 'selected' : ''}}>{{ __('Dots') }}</option>
    </select>
</div>
