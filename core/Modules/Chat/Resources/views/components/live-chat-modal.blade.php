...
<div class="chatContact" id="chatContact">
    <div class="chatContact__contents" id="chatContact__contents">
        <div class="chatContact__contents__header">
            ...
            <div class="chatContact__contents__header__login text-center {{ $auth ? "d-none" : "" }}" id="chatContact__login">
                <span class="chatContact__contents__header__say"><img src="{{ asset("assets/img/chat/wave_emoji.png") }}" alt=""></span>
                <h6 class="chatContact__contents__header__title mt-3">{{ __("Please login to chat with vendor") }}</h6>
                <p class="chatContact__contents__header__para mt-2">{{ __("Welcome! Please provide your question/issue below. We'll respond ASAP. Thank you!") }}</p>
            </div>
            <div class="chatContact__contents__header__team {{ $auth ? "" : "d-none" }}" id="chatContact__team">
        ...
        <div class="chatContact__contents__inner__form custom_form {{ $auth ? "d-none" : "" }}" id="chatContact__form">
            <form action="#">
                <div class="row g-3">
                    <div class="col-md-12">
                    ...
                    <div class="col-sm-12">
                        <div class="single_input">
                            <label for="username" class="form-label">{{ __("Phone Number / Email") }} </label>
                            <input class="form--control radius-10" type="text" id="login_phone" name="phone"
                                placeholder="{{ __("Phone Number with Country Code / Email") }}">
                            {{-- <small id="login_phone_error" class="text-danger"></small>
                            <input type="text" id="login_phone" name="phone" class="form--control radius-10" placeholder="{{ __("Phone Number with Country Code / Email") }}"> --}}
                        </div>
                    </div>
                    <div class="col-sm-12">
                        <div class="single_input">
                            <label for="password" class="form-label">{{ __("Password") }}</label>
                            <input type="password" id="login_password" name="password" class="form--control radius-10" placeholder="{{ __("Password") }}">
                        </div>
                    </div>
                    <div class="col-sm-12">
                        <div class="btn-wrapper" id="form_infoSubmit">
                            <button type="button" class="submit_btn w-100 radius-10" id="login_btn">{{ __("Login") }}</button>
                        </div>
                    </div>
                </div>
            </form>
        </div>
        <div class="chatContact__contents__inner {{ $auth ? "" : "d-none" }}" id="chatContact__inner">
        ...
        <div class="chatContact__contents__footer {{ $auth ? "" : "d-none" }}" id="chatContact__footer">
            <div class="chatContact__contents__footer__input">
                <textarea class="emoji_show" id="message" name="message" cols="100" rows="5" placeholder="{{ __("Write a message...") }}"></textarea>
            </div>
