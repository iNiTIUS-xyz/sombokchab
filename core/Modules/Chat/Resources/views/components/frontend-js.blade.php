...
<script>
    function initLiveChat(userId){
        ...
        liveChat.createChannel(userId, "{{ $vendor?->id }}","user");
        // listener
        liveChat.bindEvent('livechat-vendor-' + "{{ $vendor?->id }}", function (data){
    ...
    $(document).on("click","#open-chat-product",function (){
        if($(this).attr("data-is-user-logged-in")){
            ...
            sendMessage('','',$("#open-chat-product").attr('data-vendor-id'),"{{ $id }}");
        }
    });
    ...
    function sendMessage(message,file,vendor_id,product_id = '', disabledAppend = false){
        ...
        send_ajax_request("post", form, "{{ route("frontend.chat.message-send") }}", function (){
        }, function (response){
    ...
    function is_vendor_active(buttonElement){
        send_ajax_request("get", null,`{{ route("frontend.chat.is-vendor-active") }}/${buttonElement.attr('data-vendor-id')}`, function (){
