                ...
                <div class="card">
                    <div class="card-body">
                        <h4 class="header-title">
                            {{__("Refund Plugin License Settings")}}
                        ...
                        @endif
                        <form action="{{route('admin.refund.refund_plugin_license_update')}}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="form-group">
                                <label for="site_license_key">{{__('License Key')}}</label>
                                <input type="text" name="refund_plugin_license_key"  class="form-control" value="{{get_static_option('refund_plugin_license_key')}}" >
    ...
    (function($){
            ...
            $(document).ready(function() {
                ...
                $(document).on("click","#click_for_check_update",function (e){
                    ...
                    el.attr("disabled",true);
                    $.ajax({
                        url: "{{route('admin.refund.refund_plugin_version_check')}}",
                        type: "GET",
                        success: function (data){
                            el.children().addClass('d-none');
                            if(data.markup != ""){
