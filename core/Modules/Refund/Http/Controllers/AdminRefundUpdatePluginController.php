...
class AdminRefundUpdatePluginController extends Controller
{
    public function updatePlugin()
    {
        return view("refund::admin.update_plugin.update_plugin");
    }
    public function refund_plugin_license_settings_update(Request $request)
    {
        $request->validate([
            'refund_plugin_license_key' => 'required|string|max:191',
            'envato_username' => 'required|string|max:191',
        ]);

        $result = XgApiClient::activeLicense($request->refund_plugin_license_key,$request->envato_username);
        $type = "danger";
        ...
        if (!empty($result["success"]) && $result["success"]){
            update_static_option('refund_plugin_license_key', $request->site_license_key);
            update_static_option('refund_plugin_license_status', $result['success'] ? 'verified' : "");
            update_static_option('refund_plugin_license_msg', $result['message']);
    ...
    public function update_version_check(){
        ...
        $result = XgApiClient::checkForUpdate(get_static_option("refund_plugin_license_key"),get_static_option("refund_plugin_version",'1.0.0'));
