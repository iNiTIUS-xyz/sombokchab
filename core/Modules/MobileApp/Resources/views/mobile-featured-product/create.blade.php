                ...
                <form action="{{ route('admin.featured.product.create') }}" method="post">
                    ...
                    <div class="card-body custom__form my-5">
                        <div class="form-group">
                            ...
                            <input type="checkbox" id="category" name="category" {{ optional($selectedProduct)->type ==
                            'category' ? 'checked' : '' }} />
                        </div>
                        <div class="form-group" id="product-list" {{ optional($selectedProduct)->type == 'category' ?
                            'style=display:none' : '' }}>
                            ...
                            <select id="products" name="featured_product[]" multiple class="form-control">
                                ...
                                <option {{ in_array($product->id, json_decode(optional($selectedProduct)->ids) ?? []) ?
                                    'selected' : '' }}
                                    value="{{ $product->id }}">
                        ...
                        <div class="form-group" {{ optional($selectedProduct)->type == 'category' ? '' :
                            'style=display:none' }}
                            id="category-list">
                            ...
                            <select id="products" name="featured_category[]" multiple class="form-control select2">
                                ...
                                <option {{ in_array($category->id, json_decode(optional($selectedProduct)->ids) ?? []) ?
                                    'selected' : '' }}
                                    value="{{ $category->id }}">
