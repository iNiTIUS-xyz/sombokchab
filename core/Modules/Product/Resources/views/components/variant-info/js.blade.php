    ...
    (function($) {
        ...
        $(document).ready(function() {
            $(document).on('click', '.add_variant_info_btn', function() {
                $(this).closest('.variant_info').append(
                    `<x-product::variant-info.repeater :is-first="false" :colors="$colors" :sizes="$sizes" :all-available-attributes="$allAttributes" />`
                );
            });
        ...
        $(document).on('click', '.remove', function() {
            $(this).closest('.dataRemove').remove();
        });
        ...
        $(document).on('click', '.add_item_attribute', function(e) {
            ...
            if (attribute_name_field.val().length && attribute_value_field.val().length) {
                ...
                attribute_repeater += '<div class="form-group">';
                attribute_repeater +=
                    '<input type="text" class="form-control" name="item_attribute_value[' + container_id +
                    '][]" value="' + attribute_value + '" readonly />';
                attribute_repeater += '</div>';
                attribute_repeater += '</div>';
        ...
        $(document).on('click', '.repeater_button .add', function(e) {
            let inventory_item =
                `<x-product::variant-info.repeater :colors="$colors" :sizes="$sizes" :all-available-attributes="$allAttributes" />`;
