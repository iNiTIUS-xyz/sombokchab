...
<div class="slick-slider-items wow fadeInUp" data-wow-delay=".{{ $loop->iteration }}s">
    <div class="global-card-item vendor-global-card-item radius-10">
        ...
        <div class="global-card-contents">
            <h4 class="common-title"> <a href="{{ route('frontend.products.single', $product->slug) }}">
            ...
            <div class="d-flex flex-wrap justify-content-between">
                <div class="stock mt-2">
                    <span class="stock-available {{ $stock_count ? 'text-success' : 'text-danger' }}">
            ...
            <div class="btn-wrapper mt-3">
                @if ($product?->inventory_detail_count > 0)
                    <a href="javacript:void(0)"
                        ...
                        class="cmn-btn btn-outline-two color-two btn-small product-quick-view-ajax">
                        {{ __('Buy Now') }}
                    </a>
                @else
                    <a href="javacript:void(0)" data-id="{{ $product->id }}"
                        class="cmn-btn btn-outline-one color-one btn-small add_to_buy_now_ajax">
                        {{ __('Buy Now') }}
                    </a>
                @endif
            </div>
